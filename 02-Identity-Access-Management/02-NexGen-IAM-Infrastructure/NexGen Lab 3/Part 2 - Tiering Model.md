## Introduction
In [Part 1 - Emergency Audit](Part%201%20-%20Emergency%20Audit.md) , we audited the domain and discovered several critical vulnerabilities: Domain Admin credentials present on a standard workstation, non-gMSA service accounts, a flat OU structure, and hosts residing in the default `Computers` container.
...
The script for this step can be found [[../Ressources/Scripts/Phase2-Step1.ps1|Phase2-Step1]] .
...
The script for this step can be found [[../Ressources/Scripts/Phase2-Step2.ps1|Phase2-Step2]] .
...
<video src="https://ericyawilhit.netlify.app/videos/before-denygpo.mp4">
...
<video src="https://ericyawilhit.netlify.app/videos/after-denygpo.mp4">
  Your browser does not support the video tag.
</video>
...
1. **Deploying Group Managed Service Accounts (gMSAs):**
   * Migrate vulnerable interactive service accounts to gMSAs to enforce 120-character automatic password rotation and restrict execution strictly to designated host SPNs.
...
4. **SIEM Telemetry & Detection Engineering (Wazuh):**
   * Integrate audit logging to monitor logon attempts across tiers. Configure alerts for failed authentication attempts (Event ID 4625) and explicit privilege escalations within administrative security groups.