...
- **La Fédération SSO (Déclaration des Clients) :** Enregistrer et configurer nos deux applications cibles (Gitea pour le flux SAML et la seconde application pour le flux OIDC) en tant que "Clients" de confiance afin de matérialiser le mécanisme de session unique.
...
- **Grafana** : Une plateforme open source de visualisation et d'analyse de données
